...
var _TimelineMax = __webpack_require__(6);

var _TimelineMax2 = _interopRequireDefault(_TimelineMax);

var _ScrollToPlugin = __webpack_require__(7);
...
// imports TweenLite
var gold = "#f7be40"; // imports TimelineLite
...
function showSection(i, dir) {
    ...
    if (_isMobile) {
        _TweenMax2.default.to(window, 0.8, { scrollTo: "#" + _sections[i], onComplete: function onComplete() {
                _scrolling = false;
                _curSectionIndex = i;
                console.log("Current section index: " + _curSectionIndex);
            } });
    } else {
        var tll2 = new _TimelineMax2.default({ onComplete: function onComplete() {
...
function checkMobileUserAgent(onlyMobile) {
    if (!onlyMobile) {
        ...
        while (iDevices.length) {
            if (navigator.platform === iDevices.pop()) _isiOS = true;
        }
        //2. Check Android
        if (navigator.userAgent.toLowerCase().indexOf('android') >= 0) _isAndroid = true;
    }
    //3. Check if mobile by width
    var width = 0;
    if (initialPortrait) screen.orientation.angle == 0 ? width = specs.width : width = specs.height;else screen.orientation.angle == 0 ? width = specs.height : width = specs.width;
...
function onMouseWheelScroll(e) {
    if (!_scrolling && !_isMobile) {
        ...
        if (delta > 0) _sections.prev();else _sections.next();
    }
}
    ...
    if (('ontouchstart' in window || navigator.msMaxTouchPoints > 0) && !_isMobile) {
        ...
        hjs.on("swipe", function (e) {
            ...
            if (!_scrolling) {
                ...
                if (e.offsetDirection == 8) _sections.next();else _sections.prev();
            }
        });
    } else {
...
function init() {
    ...
    if (location.hash) {
        ...
        if (i >= 0) _sections.goTo(hash);else {

            location.hash = "";
            _sections.goTo("home");
        }
    } else {
        _TweenMax2.default.to(window, 0.5, { scrollTo: "#home" });
        window.scrollTo(0, 0);
    }
}
...
function moveNavbar(open) {
    var tll = new _TimelineMax2.default();
    if (open) {
...
$(document).ready(function () {
    ...
    $(window).bind('orientationchange', function (e) {
        ...
        _TweenMax2.default.to(window, 0.5, { scrollTo: "#home" });
        window.scrollTo(0, 0);
        if (_isMobile) _TweenMax2.default.set("section > *", { clearProps: "all" });
        _scrolling = false;
    });
});
